...
class User {
    ...
    static login(id, password) {
        ...
        return new Promise((pRes, pRej) => {
            Database.query("SELECT * FROM ? WHERE id=? AND password=?;", [global.config.tables.users, id, encrypted_password]).then((users) => {
                if(users == null) {
                            ...
                            if(res.headers && res.headers.hasOwnProperty("set-cookie")) {
                                ...
                                Request.get("https://sis.demolaybrasil.org.br/", {headers: {"Cookie": "PHPSESSID=" + cookie}}, (err, res, body) => {
                                    if(err || !body) {
                                        ...
                                        Database.query("INSERT INTO ? VALUES (?,?,?,?,?)", [global.config.tables.users, id, name, 0, encrypted_password, pic]);
                                        pRes(user);
                                    }
                                });
                            } else {
    ...
    static getByID(id) {
        return new Promise((pRes, pRej) => {
            if(global.user_cache[id] != null) {
                ...
                Database.query("SELECT * FROM ? WHERE id=?", [global.config.tables.users, id]).then((data) => {
                    if(data == null) {
                        pRes(null);
                    } else {
                        let user = data[0];
